...
exports.getEngulfingPattern = void 0;
...
function getEngulfingPattern(candles, priceChangeThreshold = 0.005) {
    ...
    if (Math.abs(lastCandle.last - lastCandle.first) / lastCandle.last < priceChangeThreshold)
        return 0;
    if (Math.abs(prevCandle.last - prevCandle.first) / prevCandle.last < priceChangeThreshold)
        return 0;
    // detect a price rise then drop
    if (lastCandle.last < lastCandle.first &&
    ...
    if (lastCandle.last > lastCandle.first &&
        ...
        if (riseRange > dropRange)
            return 1;
    }
    return 0;
}
exports.getEngulfingPattern = getEngulfingPattern;
